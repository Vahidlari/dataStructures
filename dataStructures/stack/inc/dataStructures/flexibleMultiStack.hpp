...
public:
     ...
     */
    EStackCreationResult createStack(size_t stackID, size_t initialCapacity);

    /**
     * @brief Increases the capacity of the specified stack
     * @param stackID ID of the stack to increase capacity for
     * @param newCapacity New capacity for the stack
     * @return EStackCreationResult::SUCCESS if the capacity was successfully increased, EStackCreationResult::INVALID_STACK_ID if the stack ID is invalid, EStackCreationResult::INVALID_CAPACITY if the capacity is invalid, EStackCreationResult::OVERFLOW if the stack is full
     * @note If the new capacity is less than the current size, it will return an invalid stack index.
     */
    EStackCreationResult increaseCapacity(size_t stackID, size_t newCapacity);
     ...
     */
    void push(size_t stackID, const Ttype& value);
...
private:
    ...
    struct TStackMetadata
    {
        ...
        TStackMetadata(size_t baseIndex, size_t capacity)
            : top(baseIndex), capacity(capacity), baseIndex(baseIndex), size(0) {};
        TStackMetadata() : top(0), capacity(stackInitialCapacity), baseIndex(0), size(0) {};
        size_t top;
        size_t capacity;
        size_t baseIndex;
        size_t size;
