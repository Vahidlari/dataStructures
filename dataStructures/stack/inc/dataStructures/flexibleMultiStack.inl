...
CFlexibleMultiStack<Ttype>::CFlexibleMultiStack(size_t initialCapacity)
    : data(), totalReservedCapacity(0), stackCount(0), stacks()
{
...
template <typename Ttype>
typename CFlexibleMultiStack<Ttype>::EStackCreationResult CFlexibleMultiStack<Ttype>::createStack(size_t stackID, size_t initialCapacity) {
    ...
    if (newStackBaseIndex + initialCapacity > data.capacity()) {
        // The size of the data vector is not enough to accommodate a new stack
        return EStackCreationResult::OVERFLOW;
    }

    stacks.emplace(stackID, TStackMetadata(newStackBaseIndex, initialCapacity));
    totalReservedCapacity += initialCapacity;
    return EStackCreationResult::SUCCESS;
}

template <typename Ttype>
typename CFlexibleMultiStack<Ttype>::EStackCreationResult CFlexibleMultiStack<Ttype>::increaseCapacity(size_t stackID, size_t newCapacity) {
    if (!isValidStackID(stackID)) {
        throw std::out_of_range("Invalid stack index");
    }

    if (newCapacity < stacks[stackID].size) {
        throw std::out_of_range("New capacity is less than current used size of the stack " + std::to_string(stacks[stackID].size));
    }
    ...
    totalReservedCapacity += newCapacity - stacks[stackID].capacity;
